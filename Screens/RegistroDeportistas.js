...
export const RegistroDeportistas = () => {
	...
	const onSubmit = async (values, reset) => {
		...
		for (const sKey in values) {
			...
			} else if (sKey === 'numJugador' && values.jugadorSeleccionado === 0) {
				// No agregues "numJugador" si jugadorSeleccionado es igual a "0"
			} else {
				oSend.append(sKey, values[sKey]);
			}
		}

		/* The above code is printing the value of the variable `oSend` to the console. */
		// console.log(oSend)
